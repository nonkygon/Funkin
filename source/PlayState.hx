...
class PlayState extends MusicBeatState
{
		...
		if ((upP || rightP || downP || leftP) && !boyfriend.stunned && generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				if (daNote.canBeHit && daNote.mustPress && !daNote.tooLate && !daNote.wasGoodHit)
				{
					...
					if (daNote.noteData >= 8)
					{
						trace('a mine has been added to the possible notes');
					}
				...
				if (daNote.canBeHit && daNote.mustPress && daNote.isSustainNote)
				{
					switch (daNote.noteData)
					{
						...
						case 0:
							...
							{
								goodNoteHit(daNote);
								trace('hit a left sus note');
							}
						case 1:
							if (down)
							{
								goodNoteHit(daNote);
								trace('hit a down sus note');
							}
						case 2:
							if (up)
							{
								goodNoteHit(daNote);
								trace('hit an up sus note');
							}
						case 3:
							if (right)
							{
								goodNoteHit(daNote);
								trace('hit a right sus note');
							}
					}
				}
			});
	...
	function mineHandler(note:Note)
	{
		health -= 0.50;

		trace('you hit a mine dumbass');
	...
	function mineMiss(note:Note):Void
	{
		note.isMine = false;
		goodNoteHit(note);
	}
	...
	function goodNoteHit(note:Note):Void
	{
		if (note.isMine)
		{
			trace('a mine was hit in goodNoteHit');
			mineHandler(note);
		}
